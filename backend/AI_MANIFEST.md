...
└── backend/
    ...
    │           │   ├── jobTransition.ts  # runGuardedTransition — single-write race guard for the 5 status
    │           │   │                     #   changes too cheap for a Serializable transaction (see §10)
    │           │   ├── queries/
...
> That read-then-check-then-write is three separate steps, and until 2026-07-21 the final `update()`
> in `acceptJob`, `declineJob`, `assignSitter`, `clockIn`, and `cancelJob` trusted the middle one
> unconditionally — two callers could both pass the same pre-check and the later write would win
> silently, with the loser's response describing a status that was never actually written (worst
> case: a customer's `cancelJob` and a manager's `assignSitter` racing on the same `ACCEPTED` job
> could leave it `ASSIGNED`, dispatching a sitter to a booking the customer was told was cancelled).
> `clockOut`/`completeJob`/`addTip` never had this gap — they already re-read status *inside* a
> `runSerializable` transaction because they also write the financial tables in the same breath. The
> other five don't touch money, so wrapping them in a full Serializable transaction would add retry
> and backoff cost for no reason; instead each now repeats the exact status (or, for `cancelJob`, the
> exact allowed-status set) its pre-check just verified as a condition on the `update()`'s `where`
> clause, via the shared `runGuardedTransition` helper (`job/jobTransition.ts`). Postgres matches the
> row only if it's still in that state; if a race already moved it, the guarded update matches
> nothing, Prisma raises `P2025`, and the helper converts that into a `CONFLICT` GraphQLError — the
> same code `ledger.ts` returns when a caller loses a serialization race. The ordinary pre-check is
> unchanged and still fires first, so the specific, actionable messages ("use declineJob", "contact
> the business") still cover the non-racing case; the guard only has to explain the rare genuine one.
> This is the single-write counterpart to `settleEarningsIntoPayout`'s pattern of filtering its claim
> on `payoutId: null` rather than trusting a prior read, not a new approach introduced for this.
...
- `acceptJob` / `declineJob`: caller must be an active `OWNER`/`MANAGER` of the job's business. Set `acceptedAt`/`declinedAt` respectively. Write is guarded on `status: 'PENDING'` (see the note above) — the two race each other, since `PENDING` is the one status both start from.
- `assignSitter`: caller must be an active `OWNER`/`MANAGER`. Validates `assigneeId` is an active `BusinessMember` of the **same** business as the job. Sets `assigneeId` + `assignedAt`. Write is guarded on `status: 'ACCEPTED'` — the costliest of these races to lose, since it can leave a sitter assigned to a job cancelled out from under it.
- `clockIn` / `clockOut`: caller must be the job's assigned sitter specifically — resolved by looking up the `BusinessMember` row at `job.assigneeId` and checking its `userId` matches the caller, not just any active member of the business. Set `actualStartTime`/`actualEndTime` respectively; `clockOut` also completes the job. `clockIn`'s write is guarded on `status: 'ASSIGNED'` — losing this race stranded the job, since a clock-in landing after a cancel produced an `IN_PROGRESS` job that neither `clockOut` nor `completeJob` would accept.
...
sitter's history and any future payout or cancellation-fee rule. The write is guarded on the same
`cancellable` set the pre-check built for that caller's role (not a single status, since which
statuses are legal to cancel from depends on who's asking) — see the state-machine note above.
